...
!!! tip
    Start `browse()` before creating plots that you want to retain in the gallery.
...
### Previewing the documentation over Tailscale

When the documentation preview is running on `localhost:<port>`, you can make it available to another machine on your [Tailscale](https://tailscale.com/) network with:

```sh
tailscale serve --https=<port> localhost:<port>
```

On another device connected to the same tailnet, open the HTTPS URL reported by Tailscale. Keep both the local documentation server and `tailscale serve` running while you use the preview. This exposes the documentation to your tailnet rather than directly to the public internet.
...
!!! warning
    A plot expression ending in `;` is not displayed automatically. Remove the semicolon or pass the plot to `display`.
